...
<template>
  <div id="TopNav" class="fixed bg-white z-30 flex items-center w-full border-b h-[61px]">
    ...
    <div :class="fullPath === '/' ? 'max-w-[1150px]' : ''" class="flex items-center justify-center w-full px-6 mx-auto">
      <!-- logo -->
      <div :class="fullPath === '/' ? 'w-[80%]' : 'lg:w-[20%] w-[70%]'">
        <NuxtLink to="/"><img src="~/assets/images/tiktok-logo.png" width="115"></NuxtLink>
      </div>
      ...
      <div class="flex items-center justify-end gap-3 min-w-[275px] max-w-[320px] w-full">
        ...
        <div v-if="!userStore?._id" class="flex items-center">
          <button @click="generalStore.openLogin"
            class="flex item-center bg-[#F02C56] text-white border rounded-md px-3 py-[6px]">
